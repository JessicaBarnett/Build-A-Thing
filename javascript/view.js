...
ThingView.prototype.formController = function() {
    this.clearStatus();
    ...
    if (mqPhone.matches) {
        ...
        $parentNode.append(this.formButtons(false));
    } else {
        $parentNode.append(this.formPage1());
        $parentNode.append(this.formPage2("Thing"));
        $parentNode.append(this.formButtons(false));
    }
};
...
ThingView.prototype.formButtons = function(isLastPage) {
    ...
    if (isLastPage) {
        ...
        $formContainer.children("button#make").click(makeButtonHandler);
    } else {
        $formContainer.append($('<button id="next">next!</button>'));
        $formContainer.children("button#next").click(nextButtonHandler);
    }
...
ThingView.prototype.formPage2 = function(thingType) {
    ...
    $formContainer.append($('<div id="relativeWrap"><fieldset id="radioToggle" class="group"></fieldset></div>'));

    $formContainer.find("#radioToggle").append(
        $('<input type="radio" name="isPet" value="true">'),
        $('<label for="isPet">Yes!!</label>'),
        $('<input type="radio" name="isPet" value="false">'),
        $('<label for="isPet">No</label>')
...
function nextButtonHandler() {
    if ($("div#formPage1") && !$("div#formPage2")) { //phone view page 1
        thingView.clearStatus();
        thinView.formPage2();
    } else if ($("div#formPage2")) { //phone view page 2 or tab/web view.  either way we're going to page 3
        thingView.clearStatus();
        thinView.formPage3();
    } else {
        throw new Error("this is neither form page 1 or page 2.")
    }
}
...
//present the first form

//when they choose a thing
//if a wide view, update the "what's this pet's name" piece
//if a small view show next page

//when they choose a pet status
//if yes, show human name
//if human name is filled, 
//highlight next button
//if no, highlight next button
